...
В приложении есть отдельная панель **HLAE Cinematic Cameras**. Она сохраняет текущий campath из CS2 через NetCon, поэтому команды вручную писать не нужно. Карта и настройки NetCon автоматически берутся из запущенного JTs-Hud через его локальный API.

1. В CS2 создай или загрузи campath.
2. В приложении проверь автоматически определённую карту.
3. Выбери preset: T Spawn, CT Spawn, Mid, Site и так далее. Для своей точки выбери Custom.
4. Введи имя варианта, например `dust2_t_spawn_02`.
5. В Advanced проверь поле **HLAE save folder**. По умолчанию это `game/bin/win64`, куда HLAE сохраняет XML.
6. Нажми **Save current campath**.

Приложение отправит `mirv_campath save dust2_t_spawn_02.xml`, дождётся свежего XML и скопирует его во внутреннюю библиотеку приложения по карте. В списке можно загрузить файл обратно через **Load into HLAE**, удалить его или импортировать уже существующий XML через **Import XML**. Кнопка **Export saved XMLs** экспортирует все сохранённые campaths в папки `de_dust2`, `de_mirage` и так далее.

Имя campath может содержать только латинские буквы, цифры, `_` и `-`. Повторное сохранение с тем же именем обновляет файл.

Для CS2 нужен launch option с тем же портом, который указан в настройках JTs-Hud. В твоём случае:
...
4. Aerial автоматически прочитает Telnet host и port из JTs-Hud.
