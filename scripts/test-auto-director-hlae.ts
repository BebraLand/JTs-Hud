...
const inferno = registry.load('de_inferno')
assert.ok(inferno)
assert.equal(inferno.paths.length, 11)
assert.equal(inferno.paths.filter((entry) => entry.kind === 'spawn').length, 7)
assert.ok(inferno.paths.every((entry) => entry.durationSeconds > 0))
