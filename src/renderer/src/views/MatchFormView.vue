...
const mapOptions = computed((): SelectOption[] => CS2_MAPS.map((m) => ({ value: m, label: m })))
...
const vetoTeamsOptions = computed((): SelectOption[] =>
  [
    form.value.left?.id
      ? { value: form.value.left.id, label: getTeamName(form.value.left.id) }
      : { value: '', label: '—' },
    form.value.right?.id
      ? { value: form.value.right.id, label: getTeamName(form.value.right.id) }
      : { value: '', label: '—' },
    { value: '', label: '—' }
...
<template>
  <div class="flex flex-col h-full bg-surface text-zinc-200">
    ...
    <div class="flex items-center justify-between p-6 shrink-0">
      ...
      <div class="flex items-center gap-3">
        <BaseButton variant="danger" @click="router.push('/matches')"> Cancel </BaseButton>
        <BaseButton
          v-if="!isMatReadOnly"
          @click="handleSave"
          :disabled="isSaving"
    ...
    <div
      v-if="isMatReadOnly"
      class="mx-6 mb-4 rounded-lg border border-amber-800/50 bg-amber-900/20 px-3 py-2 text-xs text-amber-300"
    >
      Read-only: this match comes from the connected MAT integration.
    </div>

    <!-- Body -->
    <fieldset :disabled="isMatReadOnly" class="flex-1 overflow-y-auto p-6 min-h-0">
      <div class="max-w-4xl mx-auto space-y-6">
          ...
          <div class="flex items-center gap-3">
            ...
            >
              <span
                ...
                :class="form.current ? 'translate-x-5' : 'translate-x-0'"
              />
            </button>
          </div>
        </div>
        ...
        <div class="grid grid-cols-2 gap-4">
          ...
          <div class="bg-zinc-800 border border-border rounded-xl p-5">
            <label
              class="block text-xs font-bold uppercase tracking-wider text-blue-400 mb-3 pb-2 border-b border-border"
              >Left Team</label
            >
            <div class="space-y-3">
              <BaseSelect
          ...
          <div class="bg-zinc-800 border border-border rounded-xl p-5">
            <label
              class="block text-xs font-bold uppercase tracking-wider text-amber-400 mb-3 pb-2 border-b border-border"
              >Right Team</label
            >
            <div class="space-y-3">
              <BaseSelect
        ...
        <div class="bg-zinc-800 border border-border rounded-xl p-5">
          <div class="flex items-center justify-between mb-4">
            <div>
              ...
              <p class="text-xs text-zinc-500 mt-0.5">
                ...
                <template v-else
                  >{{ filledCount }} / {{ SLOT_COUNT }} slots configured. Click a slot to
                  edit.</template
                >
              </p>
            </div>
          ...
          <!-- Locked state -->
          <div
            v-if="!bothTeamsSelected"
            class="flex flex-col items-center justify-center py-10 border border-dashed border-border rounded-lg text-zinc-600 gap-2"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              class="w-8 h-8"
          ...
          <div v-else class="space-y-2">
            ...
            >
              <!-- Slot number -->
              <span class="text-xs font-bold text-zinc-600 w-5 shrink-0 text-center">{{
                i + 1
              }}</span>
              ...
              <template v-if="!veto">
                ...
                <BaseButton type="button" @click="openVetoModal(i)" size="sm">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    class="w-3.5 h-3.5"
                    fill="none"
              ...
              <template v-else>
                <!-- Type badge -->
                <span
                  :class="vetoTypeColor[veto.type]"
                  class="text-[9px] font-bold uppercase tracking-wider px-2 py-1 rounded border shrink-0"
                >
                  {{ veto.type }}
                </span>

                <!-- Map name -->
                <span class="text-sm font-semibold text-text-main flex-1 truncate">{{
                  veto.mapName
                }}</span>
                ...
                <div class="flex items-center gap-2 shrink-0">
                  ...
                  <span
                    v-if="veto.side !== 'NO'"
                    class="text-xs font-bold uppercase px-1.5 py-0.5 rounded border"
                    :class="
                      veto.side === 'CT'
                        ? 'text-blue-400 border-blue-800/50 bg-blue-900/20'
                  ...
                  <span
                    v-if="veto.mapEnd"
                    class="text-xs font-bold uppercase text-emerald-400 bg-emerald-900/20 border border-emerald-800/50 px-1.5 py-0.5 rounded"
                    >Ended</span
                  >
                </div>
                ...
                <div class="flex items-center gap-1 shrink-0">
                  <button
                    ...
                    title="Edit"
                    class="p-1.5 rounded text-zinc-500 hover:text-blue-400 hover:bg-primary/10 transition-colors"
                  >
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      class="w-4 h-4"
                  ...
                  <button
                    ...
                    @click="clearSlot(i)"
                    title="Clear slot"
                    class="p-1.5 rounded text-zinc-500 hover:text-red-400 hover:bg-red-500/10 transition-colors"
                  >
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      class="w-4 h-4"
  ...
  <!-- Veto Edit Modal -->
  <div
    v-if="isVetoModalOpen"
    class="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4"
  >
    <div
      class="bg-surface rounded-xl border border-border w-full max-w-md shadow-2xl flex flex-col overflow-hidden"
    >
      <!-- Header -->
      <div class="p-5 border-b border-border flex justify-between items-center">
        <h2 class="text-base font-bold text-text-main">
          Configure Slot {{ editingSlot !== null ? editingSlot + 1 : '' }}
        </h2>
        <button
          @click="isVetoModalOpen = false"
          class="text-zinc-400 hover:text-text-main transition-colors"
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            class="h-5 w-5"
      ...
      <div class="p-5 space-y-4 overflow-y-auto">
        <!-- Map -->
        <BaseSelect v-model="vetoForm.mapName" label="Map" :options="mapOptions" size="md" />
        ...
        <div class="grid grid-cols-2 gap-3">
          <BaseSelect
            ...
            size="md"
          />
          <BaseSelect v-model="vetoForm.type" label="Type" :options="vetoTypeOptions" size="md" />
        </div>
        ...
        <button
          type="button"
          @click="showResultOverride = !showResultOverride"
          class="flex items-center gap-2 w-full group"
        >
          <div class="flex-1 border-t border-border/50"></div>
          <span
            class="text-xs font-bold uppercase tracking-wider text-amber-400 group-hover:text-amber-300 transition-colors shrink-0"
            >Result Override (optional)</span
          >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            class="w-3 h-3 text-amber-400 transition-transform shrink-0"
            :class="{ 'rotate-180': showResultOverride }"
            fill="none"
            viewBox="0 0 24 24"
            stroke="currentColor"
            stroke-width="2.5"
        ...
        <template v-if="showResultOverride">
          ...
          <div class="flex items-center gap-6">
            <BaseCheckbox
              v-model="vetoForm.reverseSide"
              label="Reverse Side"
              size="md"
              class="text-primary"
            />
            <BaseCheckbox
              v-model="vetoForm.mapEnd"
              label="Map Ended"
              size="md"
      ...
      <div class="p-5 border-t border-border flex gap-3">
        <BaseButton
          ...
          >Cancel</BaseButton
        >
        <BaseButton class="flex-1 justify-center" @click="saveVetoModal" variant="primary"
          >Save Slot</BaseButton
        >
      </div>
    </div>
  </div>
