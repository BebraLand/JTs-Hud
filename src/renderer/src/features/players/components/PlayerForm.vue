...
const handleSubmit = () => {
  if (!props.readOnly) emit('submit', form.value, avatarFile.value)
}
</script>
  ...
  <form id="playerForm" @submit.prevent="handleSubmit" class="p-6 space-y-6">
    ...
    <div class="flex gap-6 items-start">
      ...
      <div class="flex flex-col items-center gap-2 shrink-0">
        <div
          class="w-36 h-36 rounded-xl overflow-hidden border-2 border-zinc-700 bg-surface flex items-center justify-center"
        >
          <img v-if="avatarPreview" :src="avatarPreview" class="w-full h-full object-cover" />
          <img
            v-else-if="form.avatar"
            :src="resolveAssetUrl(form.avatar, assetBaseUrl)"
      ...
      <div class="flex items-center justify-between">
        <div>
          ...
          <p class="text-xs text-zinc-500 mt-0.5">
            Coaches are filtered out (hidden) from the data sent to the HUDs
          </p>
        </div>
        <BaseCheckbox v-model="form.isCoach" :disabled="readOnly" size="md" />
      </div>
    </div>
  </form>
</template>
