...
const props = defineProps<{ initialData: any; isEditing: boolean; readOnly?: boolean }>()
...
const handleSubmit = () => {
  if (!props.readOnly) emit('submit', form.value, logoFile.value)
}
</script>
  ...
  <form id="teamForm" @submit.prevent="handleSubmit" class="p-6 space-y-6">
    ...
    <div class="flex gap-6 items-start">
      ...
      <div class="flex flex-col items-center gap-2 shrink-0">
        <div
          class="w-36 h-36 rounded-xl overflow-hidden border-2 border-zinc-700 bg-surface flex items-center justify-center p-3"
        >
          <img v-if="logoPreview" :src="logoPreview" class="w-full h-full object-contain" />
          <img
            v-else-if="form.logo"
            :src="resolveAssetUrl(form.logo, assetBaseUrl)"
